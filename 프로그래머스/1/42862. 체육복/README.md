...
### 풀이 요약
- 크기가 n+1인 배열 arr을 만들어서, 번호를 인덱스로 접근하여, reserve에 있는 인덱스의 요소 값을 1로, 나머지는 0으로 설정
- 만약 lost에도 reserve에도 동일한 번호가 있다면, 자신의 것을 써야 하므로, arr에서 해당 인덱스의 요소는 0으로 한다. 그렇지 않으면 stack에 넣는다
- 제외할 때마다 lost_num을 lost배열의 크기 값에서 1씩 뺀다.
- 스택이 비워질 때까지 순회하는데, lost에 있는 값의 앞 또는 뒤의 arr값이 1이면, 빌리고, lost_num을 1 줄인다. 그리고 arr값도 0으로 바꾼다. 한번 씩 돌때마다 stack에서
하나씩 뺀다.
- 이 문제에서 핵심은 lost배열을 처음에 정렬해놓고 써야한다는 점.
- 항상 문제에서 입력 값이 정렬이 되있을거라고 장담해선 안된다.
  
### 성능 요약

메모리: 74.7 MB, 시간: 0.48 ms
