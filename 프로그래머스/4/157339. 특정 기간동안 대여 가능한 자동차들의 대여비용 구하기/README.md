...
- 문제에서는 start date와 end date를 가지고 2022년 11월에 포함하는지 여부를 묻고 있다.
- start date가 11월 30일 보다 작고, end date가 11월 1일 이후라면 11월을 거친다는 의미가 되므로, 이 조건을 이용해 구할 수 있다.
- 이를 만족하는 차량을 조회한 뒤, IN 연산자를 통해 해당 차량에 해당하지 않는 것 중에서 나머지 조건에 부합하는 조회 결과를 출력하면 된다.
