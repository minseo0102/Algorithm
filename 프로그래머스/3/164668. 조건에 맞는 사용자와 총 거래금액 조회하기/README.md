...
- 물건을 내 놓은 목록에 대한 테이블과 유저 테이블을 INNER JOIN 하여 각각의 물건 목록에 대한 USER의 정보를 매칭한다. 따라서 ON 조건은 두 테이블에 동일한 USER_ID를 비교하도록 한다
- 그 상태에서 판매 상태가 완료된 것에 대해서만 고려해야 하기 때문에 WHERE 절에 STATUS가 DONE인 것만 추리도록 한다.
- 완료된 물품에 대한 것만 추렸으면, 이제 동일한 아이디에 대해서 한 그룹으로 묶고, 그때의 판매 합계가 70만이 넘는지 확인해야 하므로, GROUP BY를 통해 USER_ID를 기준으로 그룹화한 후 HAVING을 통해 같은 그룹끼리의 SUM(PRICE)가 70만이 넘는 것만을 추리도록 한다.
- 다 추려졌으면 문제에서 주어진 마지막 조건인, 총 금액에 대한 오름차순을 만족하도록 ORDER BY 절을 통해 SUM(PRICE)를 기준으로 정렬해준다.
- 마지막에 주어진 조건을 기억하지 못해서, 다 풀어놓고 왜 결과가 틀렸는지 한참을 생각했다.
- 사소한 조건도 다시 돌아보자!!!
