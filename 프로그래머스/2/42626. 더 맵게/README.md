...
### 풀이 요약
- PriorityQueue의 기본 구현인 min heap을 사용
- 배열의 값을 mean heap에 모두 넣어 자동 정렬 시킨다
- 큐에서 두번 값을 꺼내어 식에 따라 연산하고 그 값을 다시 큐에 넣는다. answer++
- 가장 먼저 꺼내지는 값이 K이상이 될때까지 반복한다.
- 만약 큐의 크기가 1이면 그만한다
- 가장 먼저 꺼내지는 값이 K이상이면 answer를 반환하고, 그렇지 않으면 -1을 반환한다.

### 성능 요약

메모리: 124 MB, 시간: 1623.20 ms
