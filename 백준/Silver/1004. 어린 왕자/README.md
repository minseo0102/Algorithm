...
### 풀이 요약
- 출발점이 행성계 안에 있고, 도착점이 행성계 밖에 있을 때는 이탈해야 한다
- 도착점이 행성계 안에 있고, 출발점이 행성계 밖에 있을 때는 진입해야 한다
- 따라서 각 지점에서 행성계의 중심까지의 거리와 r을 비교해서 바운더리안에 있는지 확인후, 이탈의 경우 dep를 증가, 진입의 경우 en을 증가시킨다.
- 그리고 최종적으로 dep+en을 출력한다.
